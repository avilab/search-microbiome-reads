...
samples <- select(fq_ftp, experiment_title, run_accession, fastq_ftp, read_count, base_count, fastq_bytes, fastq_md5)
...
(ftp_paths <- samples %>% 
  ...
  separate(experiment_title, into = c("platform", "bio_replica"), sep = "; ") %>% 
  rename(sample = run_accession) %>% 
  select(-platform))
 
write_delim(ftp_paths, "output/samples_remote.tsv", delim = "\t")
